#!/bin/bash
# Claude Code statusline configured to match Powerlevel10k Pure style
# Minimal, clean aesthetic matching your zsh prompt
...
# ---- logging ----
{
  ...
  echo "[$TIMESTAMP] Script called from: $(pwd)"
  echo "[$TIMESTAMP] Script path: ${BASH_SOURCE[0]}"
  echo "[$TIMESTAMP] Input received: $(echo "$input" | wc -c) characters"
...
# ---- final logging ----
{
  echo "[$TIMESTAMP] Statusline output completed successfully"
  echo "[$TIMESTAMP] ========================================="
} >> "$LOG_FILE" 2>/dev/null
